...
public class CoordinatorReadClient {
     ...
     * @throws StatusRuntimeException if the RPC fails or node not found
     */
    public NodeDto getNode(String nodeId) {
        GetNodeRequest request = GetNodeRequest.newBuilder().setNodeId(nodeId).build();
        ...
        try {
            ...
            if (!response.hasNode()) {
                throw new StatusRuntimeException(io.grpc.Status.NOT_FOUND.withDescription("Node not found: " + nodeId));
            }
