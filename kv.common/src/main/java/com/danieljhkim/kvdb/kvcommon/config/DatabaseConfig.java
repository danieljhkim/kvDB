...
public class DatabaseConfig {
    ...
    private Set<String> findAvailableDatabases() {
        ...
        for (String propName : systemConfig.getAllPropertyNames()) {
            if (propName.startsWith(DB_PREFIX) && propName.contains(".url")) {
                String dbName = propName.substring(DB_PREFIX.length(), propName.length() - 4); // remove ".url"
    ...
    private void initializeDataSource(String dbName) {
        ...
        try {
            ...
            config.setMaximumPoolSize(Integer.parseInt(systemConfig.getProperty(prefix + "pool.maxSize", "10")));
            config.setMinimumIdle(Integer.parseInt(systemConfig.getProperty(prefix + "pool.minIdle", "5")));
            config.setIdleTimeout(Long.parseLong(systemConfig.getProperty(prefix + "pool.idleTimeout", "30000")));
            config.setConnectionTimeout(
                    Long.parseLong(systemConfig.getProperty(prefix + "pool.connectionTimeout", "30000")));
