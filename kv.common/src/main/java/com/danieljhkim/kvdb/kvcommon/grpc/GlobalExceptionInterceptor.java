...
/**
 * Global gRPC interceptor that catches exceptions and maps them to appropriate gRPC status codes. Handles KvException
 * hierarchy and standard Java exceptions.
 */
public class GlobalExceptionInterceptor implements ServerInterceptor {
    ...
    public static final Metadata.Key<String> SHARD_ID_KEY =
            Metadata.Key.of("x-shard-id", Metadata.ASCII_STRING_MARSHALLER);
    ...
    private Status mapExceptionToStatus(Throwable t) {
        ...
        if (t instanceof NullPointerException e) {
            return Status.INTERNAL.withDescription("Null pointer: " + e.getMessage());
        }
        ...
        if (t instanceof UncheckedIOException e) {
            return Status.INTERNAL.withDescription("I/O error: " + e.getMessage());
        }
