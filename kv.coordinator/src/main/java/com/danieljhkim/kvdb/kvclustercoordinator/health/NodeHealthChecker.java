...
/**
 * Performs health checks on storage nodes by pinging them via gRPC. Updates node status in the coordinator state
 * machine based on health check results. Reuses gRPC channels and stubs for efficiency.
 */
public class NodeHealthChecker {
    ...
    /**
     * Performs health checks on all registered nodes. Updates node status via Raft commands if status changes are
     * detected.
     */
    ...
    /**
     * Pings a node via gRPC to check if it's healthy. Reuses cached channels and stubs for efficiency.
     *
     * @param address Node address in "host:port" format
    ...
    private boolean pingNode(String address) {
        ...
        try {
            ...
            stub.withDeadlineAfter(PING_TIMEOUT_SECONDS, TimeUnit.SECONDS).ping(request);
            return true;
    ...
    /**
     * Gets or creates a gRPC stub for the given address. Channels and stubs are cached and reused.
     */
    ...
    /**
     * Removes a channel and stub from cache (e.g., when node is unhealthy). The channel will be shut down.
     */
    ...
    /**
     * Shuts down all cached channels. Should be called when the health checker is no longer needed.
     */
