...
/**
 * Immutable record representing a storage node's metadata. Used by the coordinator to track node membership and health.
 */
public record NodeRecord(
        ...
        Map<String, String> capacityHints) {
    ...
    public NodeRecord {
        ...
        capacityHints = capacityHints == null
                ? Collections.emptyMap()
                : Collections.unmodifiableMap(new HashMap<>(capacityHints));
    }
    ...
    public static NodeRecord create(String nodeId, String address, String zone) {
        return new NodeRecord(
                nodeId, address, zone, null, NodeStatus.ALIVE, System.currentTimeMillis(), Collections.emptyMap());
    }
    ...
    public NodeRecord withStatus(NodeStatus newStatus) {
        return new NodeRecord(nodeId, address, zone, rack, newStatus, lastHeartbeatMs, capacityHints);
    }
    ...
    public NodeRecord withHeartbeatAlive(long newHeartbeatMs) {
        return new NodeRecord(nodeId, address, zone, rack, NodeStatus.ALIVE, newHeartbeatMs, capacityHints);
    }
}
