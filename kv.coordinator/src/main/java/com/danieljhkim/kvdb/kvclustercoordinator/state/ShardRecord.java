...
/**
 * Immutable record representing a shard's metadata.
 * Used by the coordinator to track shard assignments and leader hints.
 */
public record ShardRecord(String shardId,long epoch,List<String>replicas,String leader,ShardConfigState configState,KeyRange keyRange){

public enum ShardConfigState {
	UNSPECIFIED, STABLE, MOVING

	}

	/**
     * Represents the key range owned by this shard.
     * Uses hash-based partitioning: startHash (inclusive) to endHash (exclusive).
     */
	 ...
	 */
	public ShardRecord{if(shardId==null||shardId.isBlank()){throw new IllegalArgumentException("shardId cannot be null or blank");}if(epoch<0){throw new IllegalArgumentException("epoch cannot be negative");}if(configState==null){configState=ShardConfigState.UNSPECIFIED;}replicas=replicas==null?Collections.emptyList():List.copyOf(replicas);}
	...
	/**
	 * Returns a new ShardRecord with updated leader hint.
	 * Only valid if the provided epoch matches the current epoch.
	 */
	public ShardRecord withLeader(long expectedEpoch, String newLeader) {
		if (this.epoch != expectedEpoch) {
			throw new IllegalArgumentException(
					"Epoch mismatch: expected " + expectedEpoch + ", current " + this.epoch);
		}
