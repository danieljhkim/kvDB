...
/**
 * Caches the cluster shard map from the coordinator. Provides thread-safe access and shard resolution for keys.
...
public class ShardMapCache implements Consumer<ShardMapDelta> {
    ...
    /**
     * -- GETTER -- Gets the last refresh timestamp.
     *
    ...
    /**
     * -- GETTER -- Gets the last delta update timestamp.
     *
    ...
    public boolean refresh() {
        ...
        if (newState != null) {
            ...
            if (oldState == null || newState.getMapVersion() > oldState.getMapVersion()) {
                ...
                logger.info(
                        "Shard map updated to version {} with {} shards and {} nodes",
                        newState.getMapVersion(),
                        newState.getShardsCount(),
                        newState.getNodesCount());
                return true;
            }
        }
        return false;
    ...
    /**
     * Consumer interface implementation for receiving delta updates. Called by WatchShardMapClient when deltas are
     * received.
     */
    @Override
    ...
    /**
     * Updates the cache from a streaming delta update. Handles both full state updates and incremental deltas.
     *
     * @param delta The delta received from the coordinator
    ...
    public boolean updateFromDelta(ShardMapDelta delta) {
        ...
        if (delta.hasFullState() && delta.getFullState().getMapVersion() > 0) {
            ...
            logger.info(
                    "Shard map updated from delta (full state) to version {} with {} shards and {} nodes",
                    fullState.getMapVersion(),
                    fullState.getShardsCount(),
                    fullState.getNodesCount());
            return true;
        }
        ...
        if (!delta.getChangedShardsList().isEmpty()
    ...
    /**
     * Schedules an asynchronous refresh of the shard map. Useful when we detect staleness but don't want to block.
     */
    ...
    /**
     * Schedules a refresh only if enough time has passed since the last refresh. This prevents thrashing when multiple
     * errors occur in quick succession.
     */
    public void scheduleRefreshIfStale() {
        ...
        if (timeSinceLastRefresh > MIN_REFRESH_INTERVAL_MS && timeSinceLastDelta > MIN_REFRESH_INTERVAL_MS) {
            logger.debug(
                    "Scheduling refresh (timeSinceLastRefresh={}ms, timeSinceLastDelta={}ms)",
                    timeSinceLastRefresh,
                    timeSinceLastDelta);
            scheduleRefresh();
        } else {
            logger.debug(
                    "Skipping refresh (too recent: timeSinceLastRefresh={}ms, timeSinceLastDelta={}ms)",
                    timeSinceLastRefresh,
                    timeSinceLastDelta);
        }
    }
    ...
    /**
     * Forces an asynchronous refresh of the shard map, bypassing refresh interval gating. This is intended for strong
     * stale-routing signals (e.g., SHARD_MOVED).
     */
     ...
     * @throws IllegalStateException if no shard map is cached
     */
