...
/**
 * Executes requests to storage nodes with retry logic, backoff, and cache invalidation. Handles gRPC errors and
 * attempts retries based on the configured RetryPolicy.
 */
public class RequestExecutor {
    ...
    public RequestExecutor(
        ...
        this.shardRoutingFailureTracker =
                shardRoutingFailureTracker != null ? shardRoutingFailureTracker : new ShardRoutingFailureTracker();
     ...
     *
     * @param shardId the shard ID for this request
     * @param isWrite true for write operations (leader only), false for reads
     * @param operation the operation to execute on the stub
     * @param nodeSupplier supplies candidate nodes for the request
            ...
            try {
                    ...
                    if (hints.leaderHint().isPresent()) {
                        ...
                        if (!failureTracker.isRecentlyFailed(hintedLeaderAddress)) {
                            try {
                                ...
                                logger.warn(
                                        "Leader-hint retry failed (node={}, code={}): {}",
                                        hintedLeaderAddress,
                ...
                if (attempt < maxAttempts) {
                    ...
                    try {
                        ...
                        return ExecutionResult.failure(
                                Status.Code.CANCELLED, "Interrupted during backoff", lastNodeAddress);
                    }
                }
            }
        }
    ...
    /**
     * Selects a node from candidates, preferring nodes that haven't failed recently.
     */
