 ...
 *
 * <p>
 * Used to validate whether this node is a replica/leader for a shard before
 * accepting writes.
 */
public class ShardMapCache implements Consumer<ShardMapDelta> {
	...
	public void accept(ShardMapDelta delta) {
		...
		if (delta.hasFullState()) {
			...
			if (updated) {
				logger.debug("Applied full-state delta version={}", delta.getFullState().getMapVersion());
			}
		}
