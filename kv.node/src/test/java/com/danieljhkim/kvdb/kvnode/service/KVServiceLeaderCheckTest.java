...
class KVServiceLeaderCheckTest {
    ...
    void set_onNonLeader_throwsNotLeader() {
        ...
        assertThrows(
                ...
                () -> svc.set(
                        KeyValueRequest.newBuilder().setKey("k").setValue("v").build(), new NoopObserver<>()));
    }
    ...
    void delete_onNonLeader_throwsNotLeader() {
        ...
        assertThrows(
                NotLeaderException.class,
                () -> svc.delete(DeleteRequest.newBuilder().setKey("k").build(), new NoopObserver<>()));
    }
    ...
    void set_whenNotReplica_throwsShardMoved() {
        ...
        var state = com.danieljhkim.kvdb.proto.coordinator.ClusterState.newBuilder()
                ...
                .putNodes(
                        "node-1",
                        com.danieljhkim.kvdb.proto.coordinator.NodeRecord.newBuilder()
                                .setNodeId("node-1")
                                .setAddress("localhost:8001")
                                .build())
                .putShards(
                        "shard-0",
                        com.danieljhkim.kvdb.proto.coordinator.ShardRecord.newBuilder()
                                .setShardId("shard-0")
                                .addReplicas("node-1")
                                .setLeader("node-1")
                                .build())
